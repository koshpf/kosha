 ...
 * On Vercel this must never throw — an unhandled middleware error 500s every page.
 */
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.error("[grok-pwa] head inject skipped:", err);
        return result;
      }
    }
    return result;
  } catch (err) {
    console.error("[grok-pwa] middleware failed:", err);
    return next();
  }
}
